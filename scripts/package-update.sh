...
jq -e '
  ...
  ([.assets[] | select(.name | test("^holla-[0-9]+[.][0-9]+[.][0-9]+-(x86_64|aarch64)-unknown-linux-gnu[.]deb$"))] | length) == 2
' "$manifest" >/dev/null
test "$(jq -r .source_ref "$identity")" = "$(jq -r .source_ref "$manifest")"
test "$(jq -r .source_digest "$identity")" = "$(jq -r .source_commit "$manifest")"

jq -S '{
  schema:"holla.apt-package-state.v1",
  source_repository,
  source_ref,
  source_commit,
  version,
  packages:(
    [.assets[] | select(.name | endswith(".deb")) | {name,sha256}]
    | sort_by(.name)
  )
